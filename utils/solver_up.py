...
from utils.statics import AverageMeter, evaluator, evaluator_ad
from utils.parser import args

__all__ = ['Trainer_up', 'Tester_up']
...
if args.quan_compander != 'no':
    _run_tag += f"_quan_{args.quan_compander}_b{args.quan_bits}"

_log_dir_base = f"data_vision/{_run_tag}"
vision_test = SummaryWriter(log_dir=f"{_log_dir_base}/test")
vision_best = SummaryWriter(log_dir=f"{_log_dir_base}/best")
vision_every = SummaryWriter(log_dir=f"{_log_dir_base}/every")
...
class Trainer_up:
    ...
    def __init__(self, model, device, optimizer, criterion, scheduler, resume=None,
                 save_path='./checkpoints', print_freq=20, val_freq=20, test_freq=20):
        ...
        self.tester = Tester_up(model, device, criterion, print_freq)
    ...
    def _iteration(self, data_loader):
        ...
        for batch_idx, (sparse_gt, uplink_CSI_mag, ) in enumerate(data_loader):
        # for batch_idx, sparse_gt in enumerate(data_loader):
            sparse_gt = sparse_gt.to(self.device)
            uplink_CSI_mag = uplink_CSI_mag.to(self.device)
            sparse_pred = self.model(sparse_gt, uplink_CSI_mag)
...
class Tester_up:
    ...
    def _iteration(self, data_loader):
        ...
        for batch_idx, (sparse_gt, uplink_CSI_mag, raw_gt) in enumerate(data_loader):
            sparse_gt = sparse_gt.to(self.device)
            uplink_CSI_mag = uplink_CSI_mag.to(self.device)
            sparse_pred = self.model(sparse_gt, uplink_CSI_mag)
            raw_gt = raw_gt.to(self.device)
            sparse_pred = sparse_pred.to(self.device)
            loss = self.criterion(sparse_pred, sparse_gt)
            flag_eigen = args.eig_flag
            flag_ad = args.ad_flag
            if flag_eigen == 0 and flag_ad == 0:
                # rho, nmse = evaluator(sparse_pred, sparse_gt, raw_gt)
                rho, nmse = evaluator(sparse_pred, sparse_gt, raw_gt, device = self.device, batch_idx = batch_idx)
            elif flag_eigen == 0 and flag_ad == 1:
                rho, nmse = evaluator_ad(sparse_pred, sparse_gt, raw_gt, device = self.device, batch_idx = batch_idx)
            elif flag_eigen == 1 and flag_ad == 0:
                rho, nmse = evaluator(sparse_pred, sparse_gt, raw_gt, device = self.device, batch_idx = batch_idx)
            elif flag_eigen == 1 and flag_ad == 1:
                rho, nmse = evaluator_ad(sparse_pred, sparse_gt, raw_gt, device = self.device, batch_idx = batch_idx)
        ...
        logger.info(f'=> Test rho:{iter_rho.avg:.3e}  NMSE: {10 * torch.log10(iter_nmse.avg):.3e}\n')
        logger.info(f'=> NMSE_all: {iter_nmse}\n')

        # print(iter_nmse)
        return iter_loss.avg, iter_rho.avg, 10 * torch.log10(iter_nmse.avg)
